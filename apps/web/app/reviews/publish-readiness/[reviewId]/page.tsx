...
import { PublishReadinessReview } from '@/components/reviews/publish-readiness-review';
import { normalizeReviewManifest } from '@/lib/reviews/publish-readiness';
...
async function readLocalManifest(reviewId: string): Promise<unknown | null> {
  ...
  const manifestPath = path.resolve(root, 'publish-readiness', reviewId, 'manifest.json');
...
async function readBlobManifest(reviewId: string): Promise<unknown | null> {
  const prefix = `reviews/publish-readiness/${reviewId}/`;
...
async function loadManifest(reviewId: string) {
  ...
  return normalizeReviewManifest(raw);
}

export default async function PublishReadinessReviewPage({ params }: PageProps) {
  const { reviewId } = await resolveParams(params);
  const manifest = await loadManifest(reviewId);

  return <PublishReadinessReview manifest={manifest} />;
}
