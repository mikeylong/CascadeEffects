#!/usr/bin/env python3
"""Build the full-track Pressure Bends intro plate scale/orientation repair."""
...
WORKFLOW = "channel_intro_pressure_bends_original_format_plate_scale_orientation_fix_v1"
OUTPUT_STEM = "channel_intro_pressure_bends_original_format_plate_scale_orientation_fix"
...
LEFT_ANCHOR_LAYOUT = {
    ...
    "therac-25": {"scale": 0.74, "offset_xy": [-95, 120], "mask_threshold": 58, "mask_expand": 19, "mask_blur": 5},
    # Review feedback: too large and facing the wrong direction in the scene family.
    "piltdown-man": {"scale": 0.70, "offset_xy": [-190, 150], "mask_threshold": 55, "mask_expand": 21, "mask_blur": 6},
}
...
def subject_alpha_mask(image: Image.Image, threshold: int, expand: int, blur: int) -> Image.Image:
    mask = image.convert("L").point(lambda value: 255 if value >= threshold else 0)
    guard = 12
    draw = ImageDraw.Draw(mask)
    draw.rectangle((0, 0, WIDTH, guard), fill=0)
    draw.rectangle((0, HEIGHT - guard, WIDTH, HEIGHT), fill=0)
    draw.rectangle((0, 0, guard, HEIGHT), fill=0)
    draw.rectangle((WIDTH - guard, 0, WIDTH, HEIGHT), fill=0)
    if expand > 1:
        mask = mask.filter(ImageFilter.MaxFilter(expand if expand % 2 else expand + 1))
    if blur > 0:
...
def apply_left_anchor_layout(image: Image.Image, slug: str) -> tuple[Image.Image, dict[str, Any]]:
    layout = LEFT_ANCHOR_LAYOUT.get(slug, {"scale": 1.0, "offset_xy": [0, 0], "mask_threshold": 58, "mask_expand": 19, "mask_blur": 5})
    scale = float(layout["scale"])
    offset_xy = [int(layout["offset_xy"][0]), int(layout["offset_xy"][1])]
    mask_threshold = int(layout.get("mask_threshold", 58))
    mask_expand = int(layout.get("mask_expand", 19))
    mask_blur = int(layout.get("mask_blur", 5))
    ...
    mask = subject_alpha_mask(image, mask_threshold, mask_expand, mask_blur).resize(scaled_size, Image.Resampling.LANCZOS)
    ...
    return canvas, {
        ...
        "read": "pass_left_plate_scaled_and_positioned_against_reference_family",
    }
...
def render_normalized_left_anchor(slug: str, source_path: Path, source_art_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    image.save(out_path)
    return out_path, {
        "normalization_model": "active_website_thumbnail_original_format_scale_orientation_v1",
        ...
        rows.append(
            {
                ...
                "read": "pass_active_website_thumbnail_source_used_for_left_plate_scale_orientation_repair",
            }
        )
    return {
        "reference_family": [
    ...
    *,
    ...
    return {
        ...
        "supersedes_local_rejected_attempt": {
            "reason": "prior packages used artifact inserts, captioned short sources, incorrect end-screen timing, wrong outro screen, or left plate perspective/orientation mismatch",
...
<head>
  ...
  <title>Pressure Bends Original-Format Plate Scale/Orientation Fix Review</title>
  <style>
...
<body>
<main>
  <h1>Pressure Bends Original-Format Plate Scale/Orientation Fix Review</h1>
  ...
  <section class="meta">
    ...
    <div><strong>Left plates</strong><br>Website thumbnails; Therac scaled down, Piltdown scaled down and mirrored</div>
...
def write_readme(package_dir: Path, final_mp4: Path, review_html: Path, manifest_path: Path, receipt_path: str) -> Path:
    ...
    readme.write_text(
        "\n".join(
            [
                "# Pressure Bends Original-Format Plate Scale/Orientation Fix",
                "",
                "Status: `review_ready_pending_human_keep`",
                "",
                "This local-review successor uses the full `The_Pressure_Bends.mp3` track, keeps the accepted timing and right-card sources, uses active website thumbnail sources for Therac-25 and Piltdown Man left plates, scales both down after review feedback, mirrors Piltdown Man to correct its stage orientation, keeps Therac-25's accepted mirrored orientation, and reserves the final 20.000s for the approved adaptive borderless titleless end screen.",
                "",
                "## Outputs",
                "",
