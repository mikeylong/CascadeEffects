#!/usr/bin/env python3
"""Build the full-track Pressure Bends intro plate edge-clean repair."""
...
WORKFLOW = "channel_intro_pressure_bends_original_format_plate_edge_clean_fix_v1"
OUTPUT_STEM = "channel_intro_pressure_bends_original_format_plate_edge_clean_fix"
...
LEFT_ANCHOR_LAYOUT = {
    ...
    "piltdown-man": {"scale": 0.70, "offset_xy": [30, 150], "mask_threshold": 55, "mask_expand": 21, "mask_blur": 6},
}
...
def render_normalized_left_anchor(slug: str, source_path: Path, source_art_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    image.save(out_path)
    return out_path, {
        "normalization_model": "active_website_thumbnail_original_format_edge_clean_v1",
        ...
        rows.append(
            {
                ...
                "read": "pass_active_website_thumbnail_source_used_for_left_plate_edge_clean_repair",
            }
        )
    ...
    *,
    ...
    return {
        ...
        "supersedes_local_rejected_attempt": {
            "reason": "prior packages used artifact inserts, captioned short sources, incorrect end-screen timing, wrong outro screen, left plate perspective/orientation mismatch, or Piltdown top/left edge border artifact",
...
<head>
  ...
  <title>Pressure Bends Original-Format Plate Edge Clean Fix Review</title>
  <style>
...
<body>
<main>
  <h1>Pressure Bends Original-Format Plate Edge Clean Fix Review</h1>
  ...
  <section class="meta">
    ...
    <div><strong>Left plates</strong><br>Therac scaled; Piltdown scaled, mirrored, edge-cleaned</div>
