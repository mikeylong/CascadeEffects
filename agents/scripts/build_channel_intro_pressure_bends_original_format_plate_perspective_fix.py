#!/usr/bin/env python3
"""Build the full-track Pressure Bends intro plate perspective repair."""
...
WORKFLOW = "channel_intro_pressure_bends_original_format_plate_perspective_fix_v1"
OUTPUT_STEM = "channel_intro_pressure_bends_original_format_plate_perspective_fix"
...
def render_normalized_left_anchor(slug: str, source_path: Path, source_art_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    image.save(out_path)
    return out_path, {
        "normalization_model": "active_website_thumbnail_original_format_grade_v1",
...
def render_body_video(proofs: list[Any], work_dir: Path, video_dir: Path) -> dict[str, Any]:
    ...
    body_video = video_dir / "channel_intro_pressure_bends_original_format_plate_perspective_fix_body_silent.mp4"
    run(
        [
            "ffmpeg",
            "-hide_banner",
...
def render_silent_video(body_video: Path, end_screen_preview: Path, video_dir: Path) -> Path:
    ...
    silent_video = video_dir / "cascade_of_effects_channel_intro_pressure_bends_original_format_plate_perspective_fix_silent.mp4"
    filter_graph = (
        ...
        rows.append(
            {
                ...
                "read": "pass_active_website_thumbnail_source_used_for_left_plate_perspective_repair",
            }
        )
    return {
        "reference_family": [
        ...
        "rows": rows,
        "read": "pass_active_website_thumbnails_used_for_therac_and_piltdown_left_plates",
...
<head>
  ...
  <title>Pressure Bends Original-Format Plate Perspective Fix Review</title>
  <style>
...
<body>
<main>
  <h1>Pressure Bends Original-Format Plate Perspective Fix Review</h1>
  ...
  <section class="meta">
    ...
    <div><strong>Left plates</strong><br>Website thumbnails; Therac mirrored</div>
...
def write_readme(package_dir: Path, final_mp4: Path, review_html: Path, manifest_path: Path, receipt_path: str) -> Path:
    ...
    readme.write_text(
        "\n".join(
            [
                "# Pressure Bends Original-Format Plate Perspective Fix",
                "",
                "Status: `review_ready_pending_human_keep`",
                "",
                "This local-review successor uses the full `The_Pressure_Bends.mp3` track, keeps the accepted timing and right-card sources, replaces Therac-25 and Piltdown Man left plates with active website thumbnail sources, mirrors Therac-25 horizontally, and reserves the final 20.000s for the approved adaptive borderless titleless end screen.",
                "",
                "## Outputs",
                "",
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    final_mp4 = video_dir / "cascade_of_effects_channel_intro_pressure_bends_original_format_plate_perspective_fix_1080p24.mp4"
