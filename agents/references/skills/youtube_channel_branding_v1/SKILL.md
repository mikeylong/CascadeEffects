...
`official research -> skill/requirements update -> implementation package -> QA review -> human keep -> contract-gated unlisted review upload when allowed -> manual YouTube Studio update`

It does not publish, change visibility, replace the channel trailer, change channel branding in YouTube Studio, or mark a candidate `keep` without human review. Programmatic upload is allowed only for an explicit `unlisted_review_upload` action when the channel-trailer production contract permits that action and the kept package has a passing explicit user approval read.

Paper Architecture is allowed here only because this skill owns YouTube channel-level branded visual assets. It must not be exported from this workflow into YouTube Shorts or long-form video source art, proof frames, final frames, cover frames, in-video end screens, or episode/video thumbnails.
...
3. Do the implementation.
   ...
   - For channel intro/trailer MP4s, a programmatic YouTube step may only be an `unlisted_review_upload` after human keep, explicit upload approval, and a passing `channel-trailer-v1` action receipt.
   - Channel trailer replacement, public/scheduled visibility, and YouTube Studio customization remain manual unless a future contract adds an explicit replacement lane.
...
## Profile Icon Edge Hygiene

For YouTube channel profile icons, prefer a square, full-bleed, alpha-free upload asset because YouTube renders the profile picture as a circle in the UI. Transparent circular PNGs and baked-in circular guide strokes can create visible halo, stair-step, or noisy edge artifacts when YouTube downscales and crops the avatar.

- Default profile-icon upload target: square PNG or JPG, no alpha channel, no transparent outside-circle region.
- Keep the CE mark centered with enough interior margin for YouTube's circular crop.
- Do not bake a visible circular guide line, border, ring, or frame into the upload asset.
- Generate review previews at `98 x 98`, `176 x 176`, and `320 x 320` on dark UI backgrounds before considering the icon usable.
- Keep transparent circular versions only as local preview/watermark variants unless Mike explicitly asks to use transparency for the channel profile picture.
...
- Use exact deterministic local text only; do not rely on generated title lettering unless Mike explicitly invokes the narrow `imagegen_title_lockup_experiment` lane.
...
- Generated title lettering is allowed only in the narrow `imagegen_title_lockup_experiment` lane and only when Mike explicitly requests ImageGen-rendered title text. It must be generated as a separate title lockup, reviewed for exact spelling and glyph readability, then composited onto a title-safe banner. Any misspelling, extra word, warped word, duplicate title, unreadable glyph, logo, label, subtitle, or watermark is `reject`.
...
- `imagegen_title_lockup_experiment`: controlled exception for an ImageGen-rendered `Cascade of Effects` title lockup composited onto an otherwise title-free banner candidate. Requires exact-title QA before compositing and remains `review_required`; failed lockups are retained only as rejected source evidence.
...
- use generated title text outside the explicit `imagegen_title_lockup_experiment` lane
- pass an ImageGen title lockup with any spelling error, extra text, duplicate text, unreadable glyph, logo, label, subtitle, watermark, or dirty chroma fringe
...
- `imagegen_title_lockup_read` when using `imagegen_title_lockup_experiment`
- `exact_title_spelling_read` when any generated title lettering is used
