...
def draw_exclusion_mask(size: tuple[int, int]) -> Image.Image:
    ...
    return block.filter(ImageFilter.MaxFilter(61)).filter(ImageFilter.GaussianBlur(2.0))
...
def make_tight_mask(source_mask_path: Path, target_mask_path: Path, qa_dir: Path) -> dict:
    ...
    hard_block = block.point(lambda v: 255 if v > 8 else 0)
    new_alpha = ImageChops.multiply(old_alpha, ImageOps.invert(hard_block))
    ...
    block_preview.putalpha(hard_block)
    ...
    route_qa = sample_routes(old_alpha, new_alpha, hard_block)
    qa = {
...
def update_player(player_path: Path, stamp: str) -> None:
    ...
    text = text.replace(
        "Challenger Living Cover Literal Rail Captions YouTube Outro Screen HTML Approval Proof",
...
def update_successor_manifest(successor_root: Path, stamp: str, qa: dict) -> None:
    ...
    for stale_key in (
        "successor_tight_matte_packet_path",
        "successor_tight_matte_manifest_path",
        "successor_tight_matte_mask_sha256",
    ):
