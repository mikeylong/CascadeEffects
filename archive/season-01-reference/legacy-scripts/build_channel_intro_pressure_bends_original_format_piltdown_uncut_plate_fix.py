#!/usr/bin/env python3
"""Build the full-track Pressure Bends intro Piltdown uncut plate repair."""
...
WORKFLOW = "channel_intro_pressure_bends_original_format_piltdown_uncut_plate_fix_v1"
OUTPUT_STEM = "channel_intro_pressure_bends_original_format_piltdown_uncut_plate_fix"
...
REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER = (
    OUTPUT_ROOT / "channel_intro_pressure_bends_original_format_plate_artifact_crop_fix_latest.json"
)
...
PILTDOWN_UNCUT_GENERATED_SOURCE = (
    OUTPUT_ROOT
    / "channel_intro_pressure_bends_original_format_piltdown_uncut_plate_fix_source_assets"
    / "piltdown-man_uncut_generated_left_anchor_source.png"
)
CURRENT_REJECTED_PILTDOWN_NORMALIZED_PLATE = (
    OUTPUT_ROOT
    / "channel_intro_pressure_bends_original_format_plate_artifact_crop_fix_20260526T230932Z"
    / "source_art/piltdown-man_website_thumbnail_left_anchor_original_format.png"
)
LEFT_ANCHOR_SOURCE_OVERRIDES = {
    "therac-25": THERAC_WEBSITE_THUMBNAIL_SOURCE,
    "piltdown-man": PILTDOWN_UNCUT_GENERATED_SOURCE,
}
LEFT_ANCHOR_GRADE = {
...
LEFT_ANCHOR_LAYOUT = {
    ...
    # Generated successor source already has uncut plinth margin and accepted orientation.
    "piltdown-man": {"scale": 1.0, "offset_xy": [0, 0], "edge_feather": 0},
}
...
def render_normalized_left_anchor(slug: str, source_path: Path, source_art_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if slug == "piltdown-man" and source_path == PILTDOWN_UNCUT_GENERATED_SOURCE:
        copied_source = source_art_dir / "piltdown-man_uncut_generated_left_anchor_source.png"
        shutil.copy2(source_path, copied_source)
        copied_source_path = str(copied_source)
        copied_source_sha256 = sha256(copied_source)
    ...
    out_path_name = (
        "piltdown-man_uncut_generated_left_anchor_original_format.png"
        if slug == "piltdown-man" and source_path == PILTDOWN_UNCUT_GENERATED_SOURCE
        else f"{slug}_website_thumbnail_left_anchor_original_format.png"
    )
    out_path = source_art_dir / out_path_name
    image.save(out_path)
    return out_path, {
        "normalization_model": "uncut_generated_original_format_plate_v1"
        if slug == "piltdown-man" and source_path == PILTDOWN_UNCUT_GENERATED_SOURCE
        else "active_website_thumbnail_original_format_artifact_crop_clean_v1",
        "source_path": str(source_path),
        "source_sha256": sha256(source_path),
        "copied_generated_source_path": copied_source_path,
        "copied_generated_source_sha256": copied_source_sha256,
        ...
        if source_path != PILTDOWN_UNCUT_GENERATED_SOURCE
        else "not_applicable_generated_uncut_successor_source",
        "read": "pass_piltdown_uncut_generated_source_used_as_left_anchor_source"
        if slug == "piltdown-man" and source_path == PILTDOWN_UNCUT_GENERATED_SOURCE
        else "pass_active_website_thumbnail_used_as_left_anchor_source",
    }
...
def build_source_proofs(source_art_dir: Path) -> tuple[list[Any], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    for row in TIMELINE:
        ...
        if slug in LEFT_ANCHOR_SOURCE_OVERRIDES:
            ...
            source_ledger[slug]["selected_left_anchor_policy"] = (
                "generated_uncut_piltdown_source_plate_png"
                if slug == "piltdown-man"
                else "active_website_episode_gallery_thumbnail_source_png"
            )
            source_ledger[slug]["left_anchor_website_thumbnail_read"] = left_anchor_reports[slug]["normalization"]["read"]
            if slug == "piltdown-man":
                source_ledger[slug]["rejected_cropped_website_thumbnail_source"] = str(PILTDOWN_WEBSITE_THUMBNAIL_SOURCE)
                source_ledger[slug]["rejected_cropped_website_thumbnail_sha256"] = sha256(PILTDOWN_WEBSITE_THUMBNAIL_SOURCE)
                source_ledger[slug]["no_current_cropped_source_reuse_read"] = (
                    "pass_current_cropped_piltdown_website_thumbnail_not_reused_as_final_plate_source"
                )
...
def make_left_plate_comparison_sheet(proofs: list[Any], qa_dir: Path) -> Path:
    ...
    for slug in ordered:
        ...
        if slug == "piltdown-man":
            role = "generated uncut source"
        elif slug in LEFT_ANCHOR_SOURCE_OVERRIDES:
            role = "active website thumbnail"
        else:
            role = "original intro reference"
...
def make_piltdown_uncut_comparison_sheet(final_mp4: Path, proofs: list[Any], qa_dir: Path) -> Path:
    by_slug = {proof.slug: proof for proof in proofs}
    piltdown_midpoint = qa_dir / "final_frame_checks/piltdown_midpoint.jpg"
    extract_frame(final_mp4, 28.229, piltdown_midpoint)
    tiles = [
        (
            "rejected Piltdown",
            CURRENT_REJECTED_PILTDOWN_NORMALIZED_PLATE
            if CURRENT_REJECTED_PILTDOWN_NORMALIZED_PLATE.exists()
            else PILTDOWN_WEBSITE_THUMBNAIL_SOURCE,
        ),
        ("generated raw", PILTDOWN_UNCUT_GENERATED_SOURCE),
        ("normalized source", Path(by_slug["piltdown-man"].base_plate_path)),
        ("final midpoint", piltdown_midpoint),
        ("Tacoma reference", Path(by_slug["tacoma-narrows"].base_plate_path)),
        ("Titanic reference", Path(by_slug["titanic"].base_plate_path)),
    ]

    cols = 3
    label_h = 46
    sheet = Image.new("RGB", (cols * 480, 2 * (270 + label_h)), (18, 20, 24))
    draw = ImageDraw.Draw(sheet)
    label_font = font(17, bold=True)
    for index, (label, path) in enumerate(tiles):
        ...
        sheet.paste(image, (x, y))
        draw.rectangle((x, y + 270, x + 480, y + 270 + label_h), fill=(18, 20, 24))
        draw.text((x + 10, y + 280), label, fill=(238, 240, 242), font=label_font)

    out_path = qa_dir / f"{OUTPUT_STEM}_piltdown_uncut_comparison_sheet.jpg"
    sheet.save(out_path, quality=92)
    return out_path
...
def bright_subject_margin_report(path: Path, *, minimum_margin_px: int = 24) -> dict[str, Any]:
    image = Image.open(path).convert("RGB").resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS)
    pixels = image.load()
    xs: list[int] = []
    ys: list[int] = []
    for y in range(0, HEIGHT, 4):
        for x in range(0, WIDTH, 4):
            ...
            if luma > 78 and (r > 92 or g > 92 or b > 92):
                ...
                ys.append(y)
    if not xs:
        return {"read": "reject_no_subject_or_plinth_pixels_detected"}
    bbox = [min(xs), min(ys), max(xs), max(ys)]
    margins = {
        "left": bbox[0],
        "top": bbox[1],
        "right": WIDTH - 1 - bbox[2],
        "bottom": HEIGHT - 1 - bbox[3],
    }
    min_margin = min(margins.values())
    return {
        "bbox_xy": bbox,
        "margins_px": margins,
        "minimum_required_margin_px": minimum_margin_px,
        "minimum_measured_margin_px": min_margin,
        "read": "pass_subject_and_plinth_clear_source_edges"
        if min_margin >= minimum_margin_px
        else "reject_subject_or_plinth_touches_source_edge",
    }
...
def left_plate_perspective_report(left_anchor_reports: dict[str, Any]) -> dict[str, Any]:
    ...
    for slug in ["therac-25", "piltdown-man"]:
        ...
        selected_source = LEFT_ANCHOR_SOURCE_OVERRIDES[slug]
        source_policy = (
            "generated_uncut_piltdown_paper_architecture_source_plate"
            if slug == "piltdown-man"
            else "active_website_episode_gallery_thumbnail_source_png"
        )
        source_margin = (
            bright_subject_margin_report(selected_source)
            if slug == "piltdown-man"
            else {}
        )
        normalized_margin = (
            bright_subject_margin_report(prepared_path)
            if slug == "piltdown-man" and prepared_path.exists()
            else {}
        )
        rows.append(
            {
                ...
                "source_sha256": sha256(selected_source),
                ...
                "source_margin_report": source_margin,
                "normalized_margin_report": normalized_margin,
                ...
                "read": "pass_piltdown_uncut_generated_source_used_for_left_plate_repair"
                if slug == "piltdown-man"
                else "pass_active_website_thumbnail_source_used_for_left_plate_artifact_crop_repair",
            }
        )
    piltdown_edge = next(row["edge_strip_report"] for row in rows if row["episode_id"] == "piltdown-man")
    piltdown_row = next(row for row in rows if row["episode_id"] == "piltdown-man")
    piltdown_source_margin = piltdown_row["source_margin_report"].get("read", "")
    piltdown_normalized_margin = piltdown_row["normalized_margin_report"].get("read", "")
    return {
        "reference_family": [
            "challenger",
            "hyatt-regency",
        ...
        "rows": rows,
        "read": "pass_therac_thumbnail_and_piltdown_uncut_generated_plate_normalized_for_left_plates",
        ...
        "piltdown_flip_read": "pass_piltdown_generated_source_already_matches_accepted_final_orientation"
        if LEFT_ANCHOR_TRANSFORMS.get("piltdown-man") == "none"
        else "reject_piltdown_generated_source_should_not_be_mirrored",
        "scale_adjustment_read": "pass_therac_scaled_down_and_piltdown_uses_uncut_source_margin",
        "piltdown_uncut_generated_plate_read": "pass_uncut_generated_source_selected_for_piltdown_left_plate",
        "piltdown_source_margin_read": "pass_raw_and_normalized_piltdown_plate_clear_frame_edges"
        if piltdown_source_margin.startswith("pass") and piltdown_normalized_margin.startswith("pass")
        else "reject_piltdown_plate_subject_or_plinth_touches_frame_edge",
        "piltdown_plinth_corner_integrity_read": "pass_piltdown_plinth_corners_not_cutoff_in_source_or_normalized_plate"
        if piltdown_source_margin.startswith("pass") and piltdown_normalized_margin.startswith("pass")
        else "reject_piltdown_plinth_corner_cutoff",
        "no_current_cropped_piltdown_source_reuse_read": "pass_current_cropped_piltdown_website_thumbnail_not_reused_as_final_plate_source"
        if LEFT_ANCHOR_SOURCE_OVERRIDES["piltdown-man"] != PILTDOWN_WEBSITE_THUMBNAIL_SOURCE
        else "reject_current_cropped_piltdown_source_reused",
...
def build_manifest(
    *,
    ...
    piltdown_uncut_comparison_sheet: Path,
    review_html: Path,
    ...
    rejected_plate_artifact_crop_fix_pointer = (
        read_json(REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER)
        if REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER.exists()
        else {}
    )
    return {
        ...
        "supersedes_local_rejected_attempt": {
            "reason": "prior packages used artifact inserts, captioned short sources, incorrect end-screen timing, wrong outro screen, left plate perspective/orientation mismatch, Piltdown edge border, Therac/Piltdown threshold-mask artifacts and crop reads, or Piltdown plinth corner cutoff",
            ...
            "review_rejected_plate_artifact_crop_fix_latest_pointer_path": str(REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER),
            "review_rejected_plate_artifact_crop_fix_latest_pointer_sha256": sha256(REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER)
            if REJECTED_PLATE_ARTIFACT_CROP_FIX_POINTER.exists()
            else "",
            "review_rejected_plate_artifact_crop_fix_pointer": rejected_plate_artifact_crop_fix_pointer,
            "review_rejected_plate_artifact_crop_fix_reason": "piltdown_left_plate_plinth_corner_cutoff",
        },
        "source_audio": {
        ...
        "reads": {
            ...
            "website_thumbnail_left_plate_read": "pass_active_website_thumbnail_retained_for_therac_only",
            ...
            "piltdown_uncut_generated_plate_read": left_plate_report["piltdown_uncut_generated_plate_read"],
            "piltdown_source_margin_read": left_plate_report["piltdown_source_margin_read"],
            "piltdown_plinth_corner_integrity_read": left_plate_report["piltdown_plinth_corner_integrity_read"],
            "no_current_cropped_piltdown_source_reuse_read": left_plate_report[
                "no_current_cropped_piltdown_source_reuse_read"
            ],
        ...
        "outputs": {
            ...
            "piltdown_uncut_comparison_sheet": str(piltdown_uncut_comparison_sheet),
            "piltdown_uncut_comparison_sheet_sha256": sha256(piltdown_uncut_comparison_sheet),
...
<head>
  ...
  <title>Pressure Bends Original-Format Piltdown Uncut Plate Fix Review</title>
  <style>
...
<body>
<main>
  <h1>Pressure Bends Original-Format Piltdown Uncut Plate Fix Review</h1>
  ...
  <section class="meta">
    ...
    <div><strong>Piltdown plate</strong><br>Generated uncut plinth; no cropped source reuse</div>
  ...
  <h2>Piltdown Uncut Plate QA</h2>
  <img src="{piltdown_uncut_comparison_sheet.relative_to(package_dir)}" alt="Piltdown rejected, regenerated, normalized, and final midpoint comparison">
  <h2>Manifest</h2>
...
def write_readme(package_dir: Path, final_mp4: Path, review_html: Path, manifest_path: Path, receipt_path: str) -> Path:
    ...
    readme.write_text(
        "\n".join(
            [
                "# Pressure Bends Original-Format Piltdown Uncut Plate Fix",
                "",
                "Status: `review_ready_pending_human_keep`",
                "",
                "This local-review successor uses the full `The_Pressure_Bends.mp3` track, keeps the accepted timing, Therac plate, right-card sources, and final 20.000s adaptive borderless titleless end screen, and replaces only the Piltdown Man left Paper Architecture plate with a generated uncut source whose skull, jaw, support post, upper plinth, lower plinth, and visible plinth corners remain inside frame.",
                "",
                "## Outputs",
                "",
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    for path, label in [
        ...
        (PILTDOWN_UNCUT_GENERATED_SOURCE, "Piltdown Man generated uncut source plate"),
    ]:
    ...
    piltdown_uncut_comparison_sheet = make_piltdown_uncut_comparison_sheet(final_mp4, proofs, qa_dir)
    ...
    review_html = make_review_html(
        ...
        piltdown_uncut_comparison_sheet,
        manifest_path,
        track_seconds,
    )
    manifest = build_manifest(
        ...
        piltdown_uncut_comparison_sheet=piltdown_uncut_comparison_sheet,
