#!/usr/bin/env python3
"""Build the full-track Pressure Bends intro timing/source review fix."""
...
WORKFLOW = "channel_intro_pressure_bends_original_format_timing_source_fix_v1"
OUTPUT_STEM = "channel_intro_pressure_bends_original_format_timing_source_fix"
...
THERAC_NORMALIZED_LEFT_ANCHOR_SOURCE = Path(
    "/Users/mike/Web_CascadeEffects/brand/assets/episode-gallery/proof-v5-ink-lit-regenerated/source-renders/"
    "therac-25-thumbnail-proof-v5-ink-lit-regenerated-source.png"
)
PILTDOWN_NORMALIZED_LEFT_ANCHOR_SOURCE = Path(
    "/Users/mike/Web_CascadeEffects/brand/assets/episode-gallery/proof-v5-ink-lit-regenerated/source-renders/"
    "piltdown-man-thumbnail-proof-v5-ink-lit-regenerated-source.png"
)
LEFT_ANCHOR_SOURCE_OVERRIDES = {
    "therac-25": THERAC_NORMALIZED_LEFT_ANCHOR_SOURCE,
    "piltdown-man": PILTDOWN_NORMALIZED_LEFT_ANCHOR_SOURCE,
}
LEFT_ANCHOR_GRADE = {
    "therac-25": {"brightness": 0.84, "contrast": 1.03, "color": 0.94},
    "piltdown-man": {"brightness": 0.80, "contrast": 1.04, "color": 0.92},
}
...
def render_normalized_left_anchor(slug: str, source_path: Path, source_art_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    out_path = source_art_dir / f"{slug}_normalized_left_anchor_original_format.png"
    image.save(out_path)
    return out_path, {
        "normalization_model": "alternate_gallery_source_original_format_grade_v1",
        ...
        "grade": grade,
        "read": "pass_normalized_to_original_intro_left_anchor_scale_orientation",
    }
...
def build_source_proofs(source_art_dir: Path) -> tuple[list[Any], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    for row in TIMELINE:
        ...
        if slug in existing:
            ...
            source_role = "new_original_format_scene_source_from_no_caption_checkpoint_and_normalized_left_anchor"
        ...
        if slug in LEFT_ANCHOR_SOURCE_OVERRIDES:
            ...
            source_ledger[slug]["left_anchor_normalization_read"] = left_anchor_reports[slug]["normalization"]["read"]
...
def render_body_video(proofs: list[Any], work_dir: Path, video_dir: Path) -> dict[str, Any]:
    ...
    body_video = video_dir / "channel_intro_pressure_bends_original_format_timing_source_fix_body_silent.mp4"
    run(
        [
            "ffmpeg",
            "-hide_banner",
...
def render_silent_video(body_video: Path, end_screen_preview: Path, video_dir: Path) -> Path:
    ...
    silent_video = video_dir / "cascade_of_effects_channel_intro_pressure_bends_original_format_timing_source_fix_silent.mp4"
    filter_graph = (
    ...
    *,
    ...
    return {
        ...
        "supersedes_local_rejected_attempt": {
            "reason": "prior packages used artifact inserts, captioned short sources, incorrect end-screen timing, or wrong outro screen",
...
<head>
  ...
  <title>Pressure Bends Original-Format Timing Source Fix Review</title>
  <style>
...
<body>
<main>
  <h1>Pressure Bends Original-Format Timing Source Fix Review</h1>
  ...
  <section class="meta">
    ...
    <div><strong>Therac/Piltdown</strong><br>No-caption CRT checkpoints</div>
...
def write_readme(package_dir: Path, final_mp4: Path, review_html: Path, manifest_path: Path, receipt_path: str) -> Path:
    ...
    readme.write_text(
        "\n".join(
            [
                "# Pressure Bends Original-Format Timing Source Fix",
                "",
                "Status: `review_ready_pending_human_keep`",
                "",
                "This local-review successor uses the full `The_Pressure_Bends.mp3` track, keeps the original intro/trailer visual grammar, swaps Therac-25 and Piltdown Man to no-caption CRT checkpoint sources, normalizes their left-anchor source art, and reserves the final 20.000s for the approved adaptive borderless titleless end screen.",
                "",
                "## Outputs",
                "",
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    for path, label in [
        ...
        (THERAC_NORMALIZED_LEFT_ANCHOR_SOURCE, "Therac-25 normalized left-anchor source"),
        (PILTDOWN_NORMALIZED_LEFT_ANCHOR_SOURCE, "Piltdown Man normalized left-anchor source"),
    ]:
    ...
    final_mp4 = video_dir / "cascade_of_effects_channel_intro_pressure_bends_original_format_timing_source_fix_1080p24.mp4"
