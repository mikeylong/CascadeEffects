...
## Key Changes
- Define a shared typography intent contract that is independent of the upstream producer.
...
- Existing still-specific commands can remain as adapters, but they should be treated as implementations of the shared contract, not the contract itself.
- The shared intent manifest should be producer-agnostic and must not require knowledge of backend-specific stages such as `final_upscale`.
...
- Video and image-sequence adapter acceptance criteria:
...
## Assumptions
- This is a shared systems spec, not a preset-specific tuning plan.
- Zero-letter generation remains the default across Cascade Effects production.
- The first concrete implementation may remain the current still adapter, but the shared contract must not encode still-only assumptions.
