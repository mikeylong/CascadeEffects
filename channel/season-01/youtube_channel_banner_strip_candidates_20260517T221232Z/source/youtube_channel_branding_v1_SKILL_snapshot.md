...
- Use little or no text unless a title-bearing review variant is explicitly useful.
- If title text is present, use exact deterministic local text only and keep it entirely in the safe area.
- Keep episode-specific objects secondary unless the user explicitly asks for a temporary campaign banner.
...
- `minimal_identity_field`: quiet, premium channel background; no title or a restrained exact title.
- `folded_cascade_world`: broad Paper Architecture landscape showing cause/effect structure without a single-episode read.
- `title_bearing_safe_area`: exact local `Cascade of Effects` title inside safe area, with crop-safe art outside.
...
- read as literal water, waterfall, river, stream, canal, or liquid
