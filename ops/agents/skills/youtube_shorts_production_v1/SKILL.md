...
- Foreground, subject, object, occlusion, cutout, and compositing masks must use `precision_matte_v1` before approval, proof promotion, or final export. Raw/imported/generated masks are `proposal_only`; final compositing alpha must be the repaired matte with raw/repaired SHA, intermediate-alpha QA, before/after edge proof, and final composite SHA. Use default inward choke `2px` plus feather `0.75px`, with `1px` allowed only when a recorded thin-detail reason proves `2px` damages required detail. Run `bin/ce validate-precision-matte --manifest PATH` for any Shorts manifest that references masks.
