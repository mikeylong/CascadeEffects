...
- Foreground, subject, object, occlusion, cutout, and compositing masks must use `precision_matte_v1` before approval or viewer-facing output. Raw/imported/generated masks are `proposal_only`; approved compositing alpha must be the repaired matte, using default inward choke `2px` plus feather `0.75px`, or `1px` only with a recorded thin-detail fallback reason. Long-form manifests that reference a mask must carry the precision matte receipt with raw/repaired SHA, intermediate-alpha QA, before/after edge proof, and final composite SHA; run `bin/ce validate-precision-matte --manifest PATH` before review or export.
...
- thumbnail/package masks use repaired `precision_matte_v1` alphas, not raw subject masks; masked thumbnail manifests must pass `bin/ce validate-precision-matte --manifest PATH`
